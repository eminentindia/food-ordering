...
if ($selectedPrice < 0 or $selectedPrice == 'NaN') {
    ...
    $count = isset($_POST['attributesData']) ? count($_POST['attributesData']) : 0;
    if ($count > 1) {
        // Loop through the attributesData array
        $attributesData = $_POST['attributesData'];
        foreach ($attributesData as $attributesData) {
            // Extract attribute data
            extract($attributesData);
            // Fetch attribute information
            $query = "SELECT * FROM dish_details WHERE dish_detail_id ='$attributeID'";
            $result = mysqli_query($conn, $query);
            $dishassocatt = mysqli_fetch_assoc($result);

            $attribute = $dishassocatt['attribute'];
            $price = $dishassocatt['price'];
            $sku = $dishassocatt['sku'];
            ...
            // Return a success message or any other response you prefer
        }
